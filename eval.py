...
generator = Generator(use_backgroud)
...
checkpoint_dir = './training_checkpoints/light'
...
def generate_output(dataset, window_size):
    ...
    #     inst = dataset[n]
        ...
        input_image = tf.reshape(input_image, (1, window_size, 256, 256, 3))
        if use_backgroud:
            noise_input = tf.random.uniform((1, 1, 1, 64))
        ...
        for i, (frame_gen, frame_tar) in enumerate(zip(gen_output[0], target)):
            diff = np.abs(frame_gen-frame_tar)
...
def get_disc_out(dataset, window_size):
    frame_count = len(dataset)+window_size
    diffs = np.zeros((frame_count, 32, 32))
    for n, inst in enumerate(tqdm(dataset)):
        input_image, target = load_test_frames(inst)
...
for i in range(36):
    # dataset_path = '../c3d/dataset/32frames/test/{:03d}_*.txt'.format(i)
    dataset_path = '../c3d/dataset/16frames_ind/test/{:03d}_*.txt'.format(i)
    ...
    if out_model:
        ...
        result_path = './results/gen_out/16frames_raw_wob16/{:03d}/'.format(i)
    else:
        diffs = get_disc_out(dataset, window_size)
        result_path = './results/disc_out/16frames_raw_wob16/{:03d}/'.format(i)
    ...
    for j, diff in enumerate(diffs):
        ...
        # div = 1
        diff = ((diff/div) * 255).astype('uint8')
